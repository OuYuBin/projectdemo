...
public class WordOperator {
 ...
 /**
	...
	public int getTableRows(int tableIndex) {
		...
		Dispatch table = Dispatch.call(tables, "Item", new Variant(tableIndex))
				.toDispatch();
		Dispatch rows = Dispatch.get(table, "Rows").toDispatch();
		rowcount = Dispatch.get(rows, "Count").toInt();
		return rowcount;
	}
	...
	public int getTableColumns(int tableIndex,int rowIndex) {
		...
		columns = Dispatch.get(cols, "Count").toInt();
		return columns;
	}
	/**
	 *  得到单元格值
 ...
 /**
	 ...
	 * @return
	 */
	public String theTextFrameFindText(String toFindText, String newText) {
		...
		for (int i = 1; i <= counts; i++) { // Item 下标从1 开始
			...
			if (hasText) {
				Dispatch TextRange = Dispatch.get(textframe, "TextRange")
						.toDispatch();
				Dispatch find = TextRange.call(TextRange, "Find").toDispatch();
				Dispatch.put(find, "Text", toFindText);
				boolean have = Dispatch.call(find, "Execute").getBoolean();
				if (have) {
					Dispatch.put(TextRange, "Text", newText); // 替换内容
   ...
   /**
	...
	public int[] getTableCellPostion(String cellValue,int tableIndex){
		int[] p=null;
		int row = getTableRows(tableIndex);
		
		for (int i = 1; i <=row; i++) {
			int col = getTableColumns(tableIndex,i);
			for (int j = 1; j <=col; j++) {
				if(getTxtFromCell(tableIndex, i, j).equals(cellValue)){
					p=new int[2];
					p[0]=i;
					p[1]=j;
					return p;
	...
	public static void main(String args[]) {
		WordOperator msWordManager = new WordOperator();
		try {
			...
			// msWordManager.openDocument("D:\\word\\智联招聘_李红辰_中文_20160613_41190447.doc");
			
			//msWordManager.saveAsHtmlFile("D:\\word\\智联招聘_李红辰_中文_20160613_41190447.doc", "D:\\word\\33");
			msWordManager.saveAsHtmlFile("D:\\word\\智联招聘_李福临_中文_20160613_47337433.doc", "D:\\word\\44");

//			int row = msWordManager.getTableRows(1);
//			int col = msWordManager.getTableColumns(1);
//			for (int i = 1; i < row; i++) {
//				for (int j = 1; j < col; j++) {
//					System.out.print(msWordManager.getTxtFromCell(1, i, j)
//							+ ",");
//				}
//			}
		//System.out.println(msWordManager.getTableCellPostion("序号", 1)[0]);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
